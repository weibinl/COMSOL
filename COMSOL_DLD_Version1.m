...
BypassTargetFlux=(BypassResult(TiltRatio,1)+fliplr([(linspace(1/TiltRatio,GapNextToBypass*(1-1/TiltRatio),TiltRatio-1)) 0]))';
BypassDifference=BypassTargetFlux-BypassResult;  %remember, the index is 1 in the bottom and getting bigger to the top
...
WallTargetDifference=WallTargetFlux-WallResult;  %remember, the index is 1 in the bottom and getting bigger to the top
...
plot(BD,'o','MarkerSize',6,'MarkerEdgeColor','b');
pause(1);
WD=norm(WallTargetDifference)
plot(WD,'^','MarkerSize',6,'MarkerEdgeColor','r');
pause(1);
if z<5
ByPassWidth=(diag(ones(TiltRatio,1)*gain1)+diag(ones(TiltRatio-1,1)*gain2,1)+diag(ones(TiltRatio-1,1)*gain2,-1)+diag(ones(1,1)*gain2,TiltRatio-1)+diag(ones(1,1)*gain2,-TiltRatio+1))*BypassDifference+ByPassWidth;
WallChannelWidth=(diag(ones(TiltRatio,1)*gain1)+diag(ones(TiltRatio-1,1)*gain2,1)+diag(ones(TiltRatio-1,1)*gain2,-1)+diag(ones(1,1)*gain2,TiltRatio-1)+diag(ones(1,1)*gain2,-TiltRatio+1))*WallTargetDifference+WallChannelWidth;
end 
ModelUtil.clear
