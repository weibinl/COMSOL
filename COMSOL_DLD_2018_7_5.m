...
LateralGap=17;
DownstreamGap=17;
TiltRatio=8; %lateral displacement vs lateral movement per bump, or # of rows required to displace one post in column
...
for i=1:1:RowNumber/2-1 
     ...
     wp2.geom.feature("Wall"+i).set('size',[500 PostDiameter+DownstreamGap*2+3]);
...
for i=1:1:RowNumber/2
    ...
    wp2.geom.feature("Wall_post"+(i-1)+RowNumber/2).set("contributeto","Array"); 
     ...
     wp2.geom.feature("Wall"+(i-1)+RowNumber/2).set('size',[500 PostDiameter+DownstreamGap+3]);
     ...
     wp2.geom.feature("Wall"+RowNumber).set('size',[500 PostDiameter+DownstreamGap+3]);
